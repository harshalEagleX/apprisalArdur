...
def validate_fha_intended_use(ctx):
    if re.search(r"intended use|intended user|HUD|FHA", _text(ctx), re.I):
        return RuleResult(rule_id="FHA-3", rule_name="FHA Intended Use and Intended User", status=RuleStatus.WARNING, message="FHA/intended-use evidence found. Verify required FHA statements.")
...
def validate_fha_repairs(ctx):
    if re.search(r"subject to repairs?|required repair|defective|safety", _text(ctx), re.I):
        return RuleResult(rule_id="FHA-6", rule_name="FHA Repairs", status=RuleStatus.WARNING, message="Repair/safety language found. Verify FHA repair treatment.")
...
def validate_fha_space_heater(ctx):
    if re.search(r"space heater", _text(ctx), re.I):
        return RuleResult(rule_id="FHA-7", rule_name="Space Heater as Primary Heat", status=RuleStatus.WARNING, message="Space heater language found. Verify FHA acceptability as primary heat.")
...
def validate_fha_security_bars(ctx):
    if re.search(r"security bars?|release latch", _text(ctx), re.I):
        return RuleResult(rule_id="FHA-8", rule_name="Security Bars on Windows", status=RuleStatus.WARNING, message="Security bar language found. Verify quick-release/code compliance.")
...
def validate_fha_attic_crawl(ctx):
    if re.search(r"attic|crawl space", _text(ctx), re.I):
        return RuleResult(rule_id="FHA-11", rule_name="Attic/Crawl Space Inspection", status=RuleStatus.WARNING, message="Attic/crawl-space evidence found. Verify head-and-shoulders inspection comments/photos.")
...
def validate_fha_well_septic(ctx):
    if re.search(r"well|septic", _text(ctx), re.I):
        return RuleResult(rule_id="FHA-12", rule_name="Well and Septic (FHA)", status=RuleStatus.WARNING, message="Well/septic language found. Verify FHA distance, hookup, and marketability requirements.")
