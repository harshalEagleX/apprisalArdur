...
def validate_location_map(ctx: ValidationContext) -> RuleResult:
    ...
    return RuleResult(rule_id="M-1", rule_name="Location Map", status=RuleStatus.PASS,
                      message="Location map evidence detected.", details={"presence_validated": True})
...
def validate_aerial_map(ctx: ValidationContext) -> RuleResult:
    ...
    return RuleResult(rule_id="M-2", rule_name="Aerial Map", status=RuleStatus.PASS,
                      message="Aerial map evidence detected.", details={"presence_validated": True})
...
def validate_plat_map(ctx: ValidationContext) -> RuleResult:
    if ctx.report.site.dimensions and ctx.report.site.shape and "irregular" not in (ctx.report.site.shape or "").lower():
        return RuleResult(
            ...
            message="Plat map not required based on extracted regular site dimensions/shape; this does not count as PASS.",
            details={"structured_validation": True},
            compared_fields=["site_dimensions", "site_shape"],
            compared_values={"site_dimensions": ctx.report.site.dimensions, "site_shape": ctx.report.site.shape},
            comparison_method="site_shape_applicability_check",
        )
    ...
    return RuleResult(rule_id="M-3", rule_name="Plat Map", status=RuleStatus.PASS,
                      message="Plat/survey evidence detected.", details={"presence_validated": True})
...
def validate_flood_map(ctx: ValidationContext) -> RuleResult:
    ...
    if re.search(r"flood map|FEMA map|flood hazard", _text(ctx), re.I):
        return RuleResult(rule_id="M-4", rule_name="Flood Map", status=RuleStatus.PASS,
                          message="Flood map/FEMA evidence detected.", details={"presence_validated": True})
