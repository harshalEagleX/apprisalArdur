...
def validate_mf_rent_matching(ctx: ValidationContext) -> RuleResult:
    ...
    if not _is_multifamily(ctx):
        return RuleResult(rule_id="MF-1", rule_name="Subject Rent Matching", status=RuleStatus.PASS,
                          message="Single-family property — rent schedule not applicable.")
...
def validate_mf_form_216(ctx: ValidationContext) -> RuleResult:
    ...
    if not _is_multifamily(ctx):
        return RuleResult(rule_id="MF-2", rule_name="Operating Income Statement", status=RuleStatus.PASS,
                          message="Single-family property — Form 216 not applicable.")
    if re.search(r"form\s*216|operating income statement", _text(ctx), re.I):
        return RuleResult(rule_id="MF-2", rule_name="Operating Income Statement", status=RuleStatus.PASS,
                          message="Form 216 evidence found.")
