...
class ExtractionService:
    ...
    def extract_and_compare(
        ...
        report.subject_section = self.extract_subject_section(extracted_text)
        
        # Step 3: Extract Contract Section (C-1 to C-5)
        report.contract_section = self.extract_contract_section(extracted_text)
        
        # Step 4: Parse engagement letter if provided
        if engagement_letter_text:
            report.engagement_letter_present = True
            report.engagement_letter = self.extract_engagement_letter(engagement_letter_text)
