...
export default function ProfilePage() {
  ...
  async function handlePwSubmit(e: React.FormEvent) {
    ...
    if (pwForm.next.length < minPwLen) {
      setPwFeedback({ type: "error", msg: `Password must be at least ${minPwLen} characters` });
      return;
    }
    setPwSaving(true);
    try {
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          <Card title="Security — change password" icon={KeyRound}>
            <form onSubmit={e => { void handlePwSubmit(e); }} className="space-y-5">
              <p className="text-sm text-slate-500">
                Use a strong password of at least {minPwLen} characters.
                Your current session will not be terminated after a change.
              </p>
          ...
          <Card title="Notification preferences" icon={Bell}>
            <p className="mb-5 text-sm text-slate-500">
              Choose which events generate notifications in the bell menu.
            </p>
            <div className="space-y-3">
              {[
                { key: "qc",     label: "QC completed",         sub: "When a batch finishes QC processing" },
                { key: "review", label: "Review notifications",  sub: "When a reviewer submits a report" },
                { key: "rerun",  label: "Re-QC updates",         sub: "When a single-file re-QC completes" },
              ].map(item => (
                <label
                  key={item.key}
