  ...
  return (
    <DeviceGate minWidth={1024} title="Document review needs a laptop or desktop width"
      ...
      <div className="foundation-grid h-screen flex flex-col bg-slate-950 text-white overflow-hidden">
        ...
        {/* All intake warnings collapsed into ONE "Before you start" card so the
            reviewer isn't met with three stacked red/amber banners. Collapsed by
            default; the count is visible and one click expands the details. */}
        {(() => {
          const allWarnings = [
            ...documentWarnings,
            ...missingDocumentWarnings,
            ...uncertainMatchWarnings,
          ];
          if (allWarnings.length === 0) return null;
          return (
            <details className="group border-b border-amber-500/25 bg-amber-950/30 px-4 py-2 text-xs text-amber-100">
              <summary className="flex cursor-pointer list-none items-center gap-1.5 font-semibold">
                <AlertTriangle size={13} />
                Before you start — {allWarnings.length} thing{allWarnings.length === 1 ? "" : "s"} to check
              ...
              <ul className="mt-2 flex flex-col gap-1 pl-1">
                {allWarnings.map((w, i) => (
                  <li key={i} className="flex gap-1.5"><span className="text-amber-400">•</span><span>{w}</span></li>
                ))}
              </ul>
            </details>
          );
        ...
        >
          ...
          {progress && progress.totalToVerify > 0 && (
            <div className="hidden md:flex items-center gap-2.5 flex-shrink-0">
              ...
              <span className={`text-[11px] font-medium ${progress.pending === 0 ? "text-green-400" : "text-slate-300"}`}>
                {progress.pending === 0
                  ? "all reviewed — ready to sign off"
                  : reviewProgress >= 50
                    ? `${reviewedCount} of ${progress.totalToVerify} reviewed — over halfway`
                    : `${reviewedCount} of ${progress.totalToVerify} reviewed`}
              </span>
