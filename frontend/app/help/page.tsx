...
const ARTICLES: Article[] = [
  {
    ...
    content: (
      <ol className="space-y-3 text-sm text-slate-300">
        ...
        <li className="flex gap-3"><span className="font-semibold text-slate-400">2.</span><span>Prepare a <strong>ZIP file</strong> containing your appraisal, engagement letter, and purchase contract in separate folders (<code className="rounded bg-muted px-1">appraisal/</code>, <code className="rounded bg-muted px-1">engagement/</code>, <code className="rounded bg-muted px-1">contract/</code>).</span></li>
    ...
    content: (
      <div className="space-y-3 text-sm">
        {[
          ...
          <div key={status} className="rounded-lg border border-white/10 bg-surface p-3">
    ...
    content: (
      <div className="space-y-4 text-sm text-slate-300">
        <div className="space-y-3 rounded-lg border border-white/10 bg-surface p-4">
          ...
          <p>Every error has a unique code shown on screen (e.g. <code className="rounded bg-muted px-1 text-xs">ERR-1042</code>). Write it down or take a screenshot.</p>
        </div>
        <div className="space-y-3 rounded-lg border border-white/10 bg-surface p-4">
          <div className="font-semibold text-white">Step 2 — Check the common causes</div>
          <ul className="space-y-1 text-slate-400 text-xs">
            <li>ZIP file is missing required folders (<code className="rounded bg-muted px-1">appraisal/</code>, <code className="rounded bg-muted px-1">engagement/</code>)</li>
  ...
  },
  {
    ...
    content: (
      <div className="space-y-4 text-sm text-slate-300">
        ...
        <div className="rounded-lg border border-white/10 bg-muted p-3 text-xs">
    ...
    content: (
      <ul className="space-y-3 text-sm text-slate-300">
        {[
          ...
          <li key={tip} className="rounded-lg border border-white/10 bg-surface p-3">
...
export default function HelpPage() {
  ...
  return (
    <div className="foundation-grid min-h-screen bg-slate-950 text-white">
      <header className="border-b border-white/10 bg-surface/80 px-6 py-5 backdrop-blur">
      ...
      <main className="max-w-3xl mx-auto p-6 space-y-3">
        ...
        {ARTICLES.map(a => (
          <div key={a.id} className="foundation-fade-in overflow-hidden rounded-lg border border-white/10 bg-surface/90 shadow-[0_16px_40px_rgba(0,0,0,0.22)]">
            <button
        ...
        {/* Still stuck */}
        <div className="foundation-fade-in rounded-lg border border-white/10 bg-surface/90 p-5 text-center shadow-[0_16px_40px_rgba(0,0,0,0.22)]">
